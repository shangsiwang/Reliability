...
\para{Define optimizing pipeline problem} The distribution of $\bv$ usually is out of the control of researchers. However, we want to find the best data collection and processing pipeline. To achieve this, we consider maximizing the discriminability of processed data, that is  
\begin{equation} 
\begin{aligned}
...
\begin{proof}[Proof of Lemma 1]
	...
	\begin{eqnarray*}  
		& &E(\hat{D}_{i,t,t'}) \\
		&=&\frac{\sum\limits_{i' \neq i}^{n} \sum\limits_{t''=1}^{s} 		    	 E(\mathbb{I}\{\delta_{i,t,t'} \leq \delta_{i,i',t,t''} \}) }{(n-1)s} \\
...
\begin{proof}[Proof of Lemma 2]
	...
	\end{eqnarray*}
	In the last step, we simplify the sum, but keep in mind that $i\neq i'$ and $t \neq t'$. We show in the previous lemma that $E(\hat{D})=D$. To demonstrate that $\hat{D}$ converges to $D$ in probability, it is suffice to show that $\text{Var}(\hat{D}) \rightarrow 0$. Since then, by Chebyshev's inequality,
	...
	\end{eqnarray*}
	As discussed above, this concludes that $\hat{D}$ converges to $D$ in probability. 
\end{proof}
