...
\begin{figure}[H]
	...
	\newline {\bf Discriminability Test Power.} One sample and two sample test power of discriminability with varying sample size is shown in the bottom right panel. The physical property and additive noise are generated from standard Gaussian distribution as described in the simulation section. At level of $0.05$, the power is estimated based on $100$ repeats. The test power becomes close to $1$ with more than $100$ samples.}
	\label{fig:fc}
\end{figure}
...
\begin{lem}	
	Under additive noise model with some regularity conditions, disrcriminability is $0.5$ implies measurements are independent of physical property, that is 
...
\begin{proof}[Proof of Lemma 3]
	Consider the additive noise setting, that is $\bx_{i,t}=\lambda \bv_{i} + \mb{\epsilon}_{i,t}$. We further assume  $\bv_{i}$ and $\mb{\epsilon}_{i,t}$ have continuous distributions; in addition,  $\bv_{i} $ has spherical distribution. We will show that $D=0.5$ implies $\lambda = 0$, hence $\bx_{i,t} = \mb{\epsilon}_{i,t}$ which is independent of physical property $\bv_{i}$ and any phenotype $\by_{i}$. First, we rewrite the definition of discriminability.
	\begin{eqnarray*}
		& &\PP(\delta_{i,t,t'} \leq \delta_{i,i',t,t''}) \\
		&=&\PP(\|\bx_{i,t}-\bx_{i,t'}\|<\|\bx_{i,t}-\bx_{i',t''}\|) \\
	...
	\end{eqnarray*}
	Let us consider the inner integral $\int_{\mathbb{S}^{d-1}}\II(\| a_1 \| < \|a_2+t\|) + \II(\| a_2 \| < \|a_1+t\||) dS$ and denote its value by $V$, we are going to show it is greater than or equal to $Area(\mathbb{S}^{d-1})$ for any $a_1$ and $a_2$. First, let us project 
	$t$ on the plane spanned by $a_1$ and $a_2$, there are three cases:
	\begin{itemize}
		\item[Case (1): ]$\|t\| \leq |\|a_1\| - \|a_2\||$, $V = Area(\mathbb{S}^{1})$
		\item[Case (2): ]$ |\|a_1\| - \|a_2\|| < \|t\| \leq |\|a_1\| + \|a_2\||$, $V = Area(\mathbb{S}^{1})$
		\item[Case (3): ]$\|t\| > |\|a_1\| + \|a_2\||$, $V =2 Area(\mathbb{S}^{1})$
	\end{itemize}
	If we don't project $t$, it only adds positive norm to the left side of the inequality of $\II(\| a_1 \| < \|a_2+t\|$ and $\II(\| a_2 \| < \|a_1+t\||)$. Therefore, the three cases become
		\begin{itemize}
			\item[Case (1): ]$\|t\| \leq |\|a_1\| - \|a_2\||$, $V = Area(\mathbb{S}^{d-1})$
			\item[Case (2): ]$ |\|a_1\| - \|a_2\|| < \|t\| \leq |\|a_1\| + \|a_2\||$, $V > Area(\mathbb{S}^{d-1})$
			\item[Case (3): ]$\|t\| > |\|a_1\| + \|a_2\||$, $V =2 Area(\mathbb{S}^{d-1})$
		\end{itemize}
	This shows $V$ always greater than  $Area(\mathbb{S}^{d-1})$. $\bA_1$ and $\bA_2$ have positive mass at any open ball centered at origin, which makes case (3) happens with positive probability. As a consequence, 
	\[\int_{} \int_{\mathbb{S}^{d-1}}\II(\| a_1 \| < \|a_2+t\|) + \II(\| a_2 \| < \|a_1+t\||) dS d\PP(a_1,a_2) >Area(\mathbb{S}^{d-1}) \]
	This shows $\PP(\| \bA_1 \| < \|\bA_2+\bV\||\|\bV \|=v) > 0.5$. As discussed above, this concludes the proof.
\end{proof}
...
\newpage
\noindent\bf{Configurations of DTI Processing Pipelines}
\begin{table}[]
	\centering
	\begin{tabular}{|l|l|l|p{3cm}|p{3cm}|}
		\hline
		\multicolumn{5}{|l|}{Registration} \\ \hline
Step       &Package & Function        & Parameters                & Description    \\ \hline
DTI De-noising     & FSL    & eddy-correct  &None specified (defaults) &   \\ \hline
MPRAGE Skull-stripping  & FSL   & bet     &`-B ` &reduction of image bias and neck voxels  \\ \hline
DTI align to MPRAGE  &FSL   &epi-reg   & None specified (defaults) &  \\ \hline
MPRAGE align to MNI 1mm & FSL   & flirt  & `-cost mutualinfo`;  `-bins 256`; `-dof 12`; `-searchrx -180 180`; `-searchry -180 180`; `-searchrz -180 180`& evaluated with mutual information, using 256 bin histograms of intensity, a 12 degree of freedom model, and searching the entire physical space for possible alignments \\ \hline
Apply MPRAGE transform to DTI & FSL &flirt  & `-interp trilinear`;  `-applyxfm& applying the transforms computed with trilinear interpolation \\ \hline
Resample aligned DTI  & nilearn &resample-img & `interpolation ="nearest"`& resampling with nearest-neighbour interpolation \\ \hline
\multicolumn{5}{|l|}{Diffusion processing} \\ \hline
Step    & Package & Function   & Parameters  & Description \\ \hline
Tensor Fitting   & dipy & TensorModel & None specified (defaults) & \\ \hline
